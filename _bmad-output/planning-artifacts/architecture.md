...
altitude: feature
paradigm: 'pipes-and-filters (pipeline modulaire) + RPC léger'
scope: 'Application web auto-hébergée Poke-Radar : pipeline détection → scoring → notification, exposition HTTP, déploiement VPS'
status: final
created: '2026-07-22'
updated: '2026-07-23T11:30'
binds:
  - FR-01..FR-09
  - NFR-01..NFR-07
sources:
...
quality_gate:
  adversarial: '2026-07-22 — PASS, 0 holes, 1 clarification mineure (appliquée)'
  rubric: '2026-07-22 — PASS, 0 critical, 1 medium (appliqué), 0 low'
  version_check: '2026-07-22 — ⚠️ CAUTION, versions non vérifiées (à confirmer avant implémentation)'
  prd_crosscheck: '2026-07-22 — ✅ ALIGNED, 3 points discutés et résolus'
companions: []
---

# Architecture Spine — Poke-Radar
...
- **Prevents:** dépendance à une base externe (PostgreSQL, MySQL) ; perte de données lors des mises à jour ; race condition entre migrations et lancement du scheduler/serveur HTTP.
- **Rule:** SQLite est la seule base de données. Le schéma est versionné via migrations, appliquées séquentiellement au démarrage du processus unique — **avant** le lancement du scheduler (`tokio::spawn`) et du serveur HTTP (`axum::serve`). L'ordre de démarrage est : init config → open/migrate DB → spawn scheduler → bind HTTP → serve. Tous les timestamps sont UTC ISO-8601. Tous les montants sont en centimes (`i64`). Les calculs de marge sont persistés avec leur détail pour explicabilité. Indexation ciblée sur les colonnes utilisées par le dashboard et la déduplication d'alertes.
...
- **Binds:** FR-01..FR-09, NFR-07, all user journeys
...
| State & cross-cutting (mutation, errors, logging, config, auth) | Mutation d'état via le backend uniquement (pas d'écriture directe SQLite par le frontend). Logging structuré via `tracing` (niveaux : error, warn, info, debug). Configuration : variables d'environnement exclusivement. Auth : header `Authorization: Bearer <token>` sur chaque appel API. Séquence de démarrage stricte : init config → open/migrate DB → spawn scheduler → bind HTTP → serve. |
...
> ⚠️ Les versions marquées `latest stable` ou sans numéro précis sont à vérifier avant implémentation. Les versions numérotées sont des hypothèses (recherche web indisponible lors de la rédaction). À confirmer via `cargo search` et `npm info` avant le premier `cargo build` / `npm install`.
...
| NFR-07 — Qualité et tests | Tests unitaires (AD-13), intégration (AD-14), frontend (AD-15), E2E (AD-16) | AD-13, AD-14, AD-15, AD-16 |
...
## Quality Gate — Synthèse des reviews (2026-07-22)

### Adversarial Review — ✅ PASS, 0 holes

6 tentatives de divergence testées. Aucun trou trouvé. 1 clarification mineure identifiée : expliciter l'ordre de démarrage dans AD-5 (migrations avant scheduler et serveur HTTP) → **appliqué** dans la règle AD-5 ci-dessus.

### Rubric Review — ✅ PASS, 1 finding (appliqué)

18 ADs couvrent tous les points de divergence. 1 finding MEDIUM : timing des migrations non explicite dans AD-5 → **appliqué** (séquence de démarrage ajoutée à AD-5 et aux Consistency Conventions).

### Version Check — ⚠️ CAUTION

Les versions de la stack n'ont pas pu être vérifiées (recherche web indisponible lors de la rédaction). L'avertissement est conservé dans la table Stack. À résoudre avant le premier `cargo build` / `npm install`.

### PRD ↔ Architecture Cross-check — ✅ ALIGNED

3 points discutés avec John (PM), tous résolus :
1. 🔴 `architecture.md` marqué comme superseded → **résolu** par cette consolidation.
2. 🟡 Terminologie PRD : "REST ou similaire" → le PRD a été mis à jour avec "API HTTP RPC".
3. 🟢 SSE absent du PRD → le PRD FR-06 mentionne maintenant explicitement les Server-Sent Events.

---

## Historique

- **2026-02-19** — Architecture initiale Tauri desktop (ADR-001 à ADR-006)
- **2026-07-22** — Pivot web : nouveau spine (AD-1 à AD-18), architecture Tauri obsolète
- **2026-07-23** — Consolidation : fusion du dossier shardé + reviews dans le document canonique unique `architecture.md`. Fix AD-5 (timing des migrations). Ancien dossier shardé archivé dans `architecture/_archive/`.
